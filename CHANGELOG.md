...
这次把“额度周期”和“个人使用记录”真正分开了。

### 修正统计方式

- 按下 Reset 后，只有剩余额度、理想用量线和下次 Reset 时间会重新计算。
- 运行时间、任务数、任务闭环率和 Token 改为固定统计周一到周日，不会再因为中途 Reset 而清零。
- 每日 Token 图改回星期一到星期日，因此同一个星期里的历史柱子会一直保留。
- 预测值现在代表照目前速度到星期日大概会达到多少；比较对象则是上一个完整自然周。
- 修复系统同时注册多个旧 Widget 后，Small 和 Large 尺寸可能空白的问题；修复工具现在会先注销重复版本。

### 整理中间版面

- 取消会跟着参考线移动、容易挤在一起的数字。
- 每一项统一分成三列：`SO FAR` 是本周目前的数据，`WEEK EST.` 是周日预测，`LAST WEEK` 是上周结果。
- 长条里的绿色或蓝色仍代表本周已经完成的部分；黄色虚线代表本周预测，白线代表上周结果。
